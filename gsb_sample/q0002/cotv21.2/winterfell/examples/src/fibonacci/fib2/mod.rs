...
use super::utils::{compute_fib_term, compute_fib_term_with_init};
...
impl<H: ElementHasher> FibExample<H> {
    pub fn new(
        ...
        let a0 = a0.map(BaseElement::new).unwrap_or(BaseElement::ONE);
        let a1 = a1.map(BaseElement::new).unwrap_or(BaseElement::ONE);

        // compute Fibonacci sequence
        let now = Instant::now();
        let result = compute_fib_term_with_init(a0, a1, sequence_length);
        println!(
    ...
    H: ElementHasher<BaseField = BaseElement> + Sync,
{
    fn prove(&self) -> Proof {
        ...
        println!("n: {}, a0: {}, a1: {}", self.sequence_length, self.a0, self.a1);
