...
fn main() {
    ...
    let example = match options.example {
        ExampleType::Fib {
            ...
            // Check that a0 and a1 are either both provided or both omitted
            if a0.is_some() != a1.is_some() {
                eprintln!("Error: --a0 and --a1 must be provided together. Either both or neither should be specified.");
                std::process::exit(1);
            }
            fibonacci::fib2::get_example(&options, sequence_length, a0, a1)
        },
